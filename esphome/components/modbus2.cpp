...
static const char *const TAG = "modbus2";
...
bool Modbus2::parse_modbus_byte_(uint8_t byte) {
  ...
  if (((function_code >= 65) && (function_code <= 72)) || ((function_code >= 100) && (function_code <= 110))) {
    ...
    ESP_LOGD(TAG, "FUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU");
    uint16_t computed_crc = crc16(raw, data_offset + data_len);
    uint16_t remote_crc = uint16_t(raw[data_offset + data_len]) | (uint16_t(raw[data_offset + data_len + 1]) << 8);
    if (computed_crc != remote_crc) {
...
float Modbus2::get_setup_priority() const {
  // After UART bus
  return setup_priority::BUS - 1.0f;
}

void Modbus2::send(uint8_t address, uint8_t function_code, uint16_t start_address, uint16_t number_of_entities,
...
void Modbus2::send_raw(const std::vector<uint8_t> &payload) {
  if (payload.empty()) {
    return;
  }
